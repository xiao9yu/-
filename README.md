...
**质量基线**：后端 323 个自动化用例全绿（`pytest`，2 deselected；2026-09-23）；RAG 检索评测 hit@5 **96.7%**、MRR **0.869**、引用合法率 **100%**（`backend/eval/` 可复现）。
...
- **知识库出题**：生成类型新增「知识库出题」——检索公共库+私有库资料、仅依据资料出题、校验后自动进入该课程学习题库（同题干去重追加）。
...
- **三学习方向**：人工智能导论 / 数据结构与算法 / 机器学习，顶部选择器一键切换，画像/诊断/练习/错题本按方向独立（跨方向同名知识点互不影响）。
- **演示数据**：seed 脚本预置 3 个方向的知识图谱（每方向 12 个知识点 + 前置关系）与习题集（每方向 72 道手工种子题：12 知识点 × 易2/中2/难2，共 216 题保底；答案字母 A/B/C/D 均衡分布，杜绝全选同一字母可全对）。
- **题目三通道**：① 教师备课生成习题/月考题选择题自动入练习池 ② 备课「知识库出题」——检索知识库资料、仅依据资料出题、自动进入该课程学习题库 ③ 手工种子 216 题保底；练习「下一题」自动避开上一题（prev_stem）。
...
- **V1 范围说明**：知识图谱本机未装 Neo4j，按工单 16 退化方案用 SQL 表 + networkx 实现。
...
当前基线：**323 passed, 2 deselected**（2026-09-23 实测）。跑测前建议 `set HF_HUB_OFFLINE=1` 走本地模型缓存。
